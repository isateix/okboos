...
export default function Secao11() {
  ...
  const produtosVisiveis = products.slice(startIndex, endIndex);
  ...
  const handleNext = () => {
    setStartIndex((prev) =>
      Math.min(prev + itensPorPagina, products.length - itensPorPagina)
    );
  };
  ...
  return (
    <section className="my-16 p-6 bg-gray-100 w-full relative overflow-hidden">
      ...
      <div className="relative flex items-center justify-center">
        ...
        {/* Produtos visíveis */}
        <div
          className="flex gap-8 justify-center transition-all duration-300"
          style={{
        ...
        >
          {produtosVisiveis.map((produto) => (
            <div
              key={produto.id}
              className="flex flex-col items-center w-64 h-64 border border-transparent rounded-lg p-5 bg-white shadow-sm"
            >
              ...
              <span
                className="text-lg mt-4 text-center font-medium overflow-hidden text-ellipsis text-blue-900"
                style={{
                  display: "-webkit-box",
                  WebkitLineClamp: 2,
        ...
        <button
          onClick={handleNext}
          disabled={startIndex + itensPorPagina >= products.length}
